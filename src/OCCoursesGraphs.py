...
from OCCoursesConfig import pd, Network, OCGraphsIconsURL, math, np, VizTemplate
...
def mini_graph(text, height, width, bgcolor, font_color, directed, notebook, layout):
    g = Network(height=height, width=width, bgcolor=bgcolor, font_color=font_color, directed=directed,
                notebook=notebook, layout=layout)
    g.add_node(0, "OpenClassrooms", group="root", color="#7451eb", shape="image",
               image=OCGraphsIconsURL["OpenClassrooms"], physics=False)
...
def empty_graph(height, width, bgcolor, font_color, directed, notebook, layout):
    ...
    g.add_node(0, "OpenClassrooms", group="root", color="#7451eb", shape="image",
               image=OCGraphsIconsURL["OpenClassrooms"], physics=False)
    g.add_node(1, "Nothing to show!", group="root", color="#C951eb", title="404 : Nothing found!", shape="text",
               physics=False)
    color = "#B491FE"
...
def build_topics_and_paths_graph(height='640px', width='99%', bgcolor="#ffffff",
                                 font_color=False, heading=None, show_titles=True, directed=False, notebook=True,
                                 layout=False, show_buttons=False, filter_options=None):
    magic_coeff = 0.618
    #     filter_options={
    ...
    dfp = ocd.OC_Paths[["topic_id", "path_id", "path_title", "path_language", "path_level", "path_duration_months",
                        "path_illustration"]].copy()
    ...
    # .drop_duplicates()
    lang_nodes = dftp.sort_values(by=["topic_id", "path_language"])[
        ["topic_lang_uid", "path_language"]].drop_duplicates()
    ...
    path_nodes = dftp[
        ["path_uid", "path_title", "path_grade", "path_duration_months", "path_illustration"]].sort_values(
        by="path_grade").drop_duplicates()
    ...
    if (heading is None):
        ...
        g = Network(height=height, width=width, bgcolor=bgcolor, font_color=font_color, directed=directed,
                    notebook=notebook, layout=layout, heading=heading)
    ...
    # ------- ROOT NODE
    g.add_node(0, "OpenClassrooms", group="root", color="#7451eb", shape="image",
               image=OCGraphsIconsURL["OpenClassrooms"], mass=3, physics=False)
    ...
    for i, n in topic_nodes.iterrows():
        ...
        if (previous_nid == 0):
            ...
            if len(topic_nodes) > 3:
                color = "#B491FE"
                g.add_edge(previous_nid, int(n["nid"]), hidden=False, length=120,
    ...
    for i, n in lang_nodes.iterrows():
        ...
        g.add_node(int(n["nid"]), n["label"], group="lang", size=16, color="#B491FE", shape="image",
                   image=OCGraphsIconsURL[n["label"]], physics=True, x=px, y=py)
    ...
    for i, e in topic_lang_edges.iterrows():
        ...
        nb_paths = len(dftp[dftp["topic_id"].isin([topic_id])])
        color = "#B491FE"
        g.add_edge(int(e["src_nid"]), int(e["tgt_nid"]),
                   length=(ray_topic_lang * magic_coeff) + (nb_paths * (1 + magic_coeff) * 10),
                   color={'color': color, 'inherit': 'false'})
    ...
    if len(topic_nodes) > 3:
        ...
        lang_distance_iso = int(
            lang_distance_iso / 3)  # we want more distance between path clusters, and more density in the clusters
        ...
        for i, n in topic_nodes.iterrows():
            ...
            for j, e in d.iterrows():
                if (previous_nid == 0):
                    ...
                    color = "#B491FE"
                    g.add_edge(previous_nid, nid, hidden=False, length=length, title=str(length),
                               color={'color': color, 'inherit': 'false'})
                    previous_nid = nid
    ...
    for i, n in path_nodes.iterrows():
        ...
        if (show_titles):
            g.add_node(int(n["nid"]), n["label"], title=n["uid"], size=int(n["size"]), group="path", shape="image",
                       image=ic)
        else:
            g.add_node(int(n["nid"]), n["label"], size=int(n["size"]), group="path", shape="image",
                       image=ic)  # value = int(n["value"]),
...
def add_pseudo_nodes_between(g, aia, src_nid, tgt_nid, src_uid, tgt_uid, parenthood_floor, parenthood, color,
                             show_titles):
    short_link = 40
    long_link = 450
...
def build_path_projects_courses_graph(path_id, max_depth=1, i_read_my_courses_once=True,
                                      show_only_requirement_relations=True,
                                      show_relations_max_distance=0, create_pseudo_nodes_over_max_distance=True,
                                      show_my_way=True,
                                      height='640px', width='99%', bgcolor="#ffffff",
                                      font_color=False, heading=None, shape="mountain", directed=False, notebook=True,
                                      layout=False,
    ...
    skeleton_edges = pd.DataFrame(
        columns=["src_uid", "src_nid", "relation", "tgt_uid", "tgt_nid", "parenthood", "parenthood_uid"])
    ...
    dfpc = ocd.OC_ProjectsCoursesLinks[
        ocd.OC_ProjectsCoursesLinks["path_id"].isin([path_id])]  # courses referenced/required by the project
    #     display(dfpc.head())
    ...
    while linked_courses_found and depth < max_depth:
        ...
        depth += 1
        df = ocd.OC_CoursesLinks[
            (ocd.OC_CoursesLinks["src_course_id"].isin(courses)) & ~(ocd.OC_CoursesLinks["tgt_course_id"].isin([0]))]
    ...
    #     display(dfc)
    dfcc = ocd.OC_CoursesLinks[(ocd.OC_CoursesLinks["src_course_id"].isin(courses)) & (
        ocd.OC_CoursesLinks["tgt_course_id"].isin(courses))].drop_duplicates(
        subset=["src_course_id", "tgt_course_id", "relation"])
    ...
    # pick what we need 
    # paths
    dfpa = dfpa[["topic_id", "path_id", "path_title", "path_language", "path_level", "path_duration_months",
                 "path_illustration"]].copy()
    # projects
    dfpr = dfpr[["path_id", "project_number", "project_title", "project_description", "project_duration_hours",
                 "status"]].sort_values(by="project_number").copy()
    # projects-courses edges

    dfpc = dfpc.merge(dfoccourses, on='course_id', how='left')
    dfpc = dfpc[['path_id', 'project_number', 'course_id', 'course_title', 'course_difficulty', 'course_duration_hours',
                 'course_url', 'status']].copy()
    # courses
    dfc = dfc[['topic_id', 'course_id', 'course_title', 'course_difficulty', 'course_duration_hours', 'course_url',
               'status']].copy()
    ...
    dfcc = dfcc[['src_course_id', 'relation', 'tgt_course_id', 'course_title', 'course_url', 'course_difficulty',
                 'course_duration_hours', 'status']].copy()
    dfcc.columns = ['src_course_id', 'relation', 'tgt_course_id', 'src_course_title', 'src_course_url',
                    'src_course_difficulty', 'src_course_duration_hours', 'src_status']
    dfcc = dfcc.merge(dfoccourses, left_on='tgt_course_id', right_on='course_id', how='left')
    dfcc = dfcc[
        ['src_course_id', 'relation', 'tgt_course_id', 'src_course_title', 'src_course_url', 'src_course_difficulty',
         'src_course_duration_hours', 'src_status', 'course_title', 'course_url', 'course_difficulty',
         'course_duration_hours', 'status']].copy()
    dfcc.columns = ['src_course_id', 'relation', 'tgt_course_id', 'src_course_title', 'src_course_url',
                    'src_course_difficulty', 'src_course_duration_hours', 'src_status', 'tgt_course_title',
                    'tgt_course_url', 'tgt_course_difficulty', 'tgt_course_duration_hours', 'tgt_status']
    # add unique ids and other columns for the graph
    ...
    link_colors = {
        ...
        "primary_references": "#22D",
        # the "other" links between a project and courses (the list is empty but we have a color for that :) )
    ...
    # --------- PATH
    if (show_titles):
        g.add_node(path["nid"], path["path_title"], group="root", color=path_color, shape="image",
                   image=OCGraphsIconsURL["path"], physics=False, x=px, y=py, title=str(path["path_title"]))
    else:
        g.add_node(path["nid"], path["path_title"], group="root", color=path_color, shape="image",
                   image=OCGraphsIconsURL["path"], physics=False, x=px, y=py)
    # skeleton_nodes = skeleton_nodes.append({"uid": path["uid"],"nid" : path["nid"]}, ignore_index = True)
    skeleton_nodes = pd.concat([skeleton_nodes, pd.DataFrame([{"uid": path["uid"], "nid": path["nid"]}])],
                               ignore_index=True)
    ...
    for i, n in dfpr.iterrows():
        ...
        if (show_titles):
            g.add_node(int(n["nid"]), str(n["project_number"]) + "-" + n["project_title"], color=project_color,
                       group="project", shape="image", \
                       image=image, physics=False, x=px, y=py,
                       title=str(path["path_title"]) + " / " + str(n["project_title"]))
        else:
            g.add_node(int(n["nid"]), str(n["project_number"]) + "-" + n["project_title"], color=project_color,
                       group="project", shape="image", \
                       image=image, physics=False, x=px, y=py)
        ...
        skeleton_nodes = pd.concat([skeleton_nodes, pd.DataFrame([{"uid": n["uid"], "nid": n["nid"]}])],
                                   ignore_index=True)
        ...
        if (previous_nid == 0):
            ...
            skeleton_edges = pd.concat([skeleton_edges, pd.DataFrame([{"src_uid": path["uid"], "src_nid": path["nid"],
                                                                       "relation": "precedes", "tgt_uid": n["uid"],
                                                                       "tgt_nid": n["nid"]}])], ignore_index=True)
            previous_uid = n["uid"]
            previous_nid = int(n["nid"])
        else:
            ...
            skeleton_edges = pd.concat([skeleton_edges, pd.DataFrame([{"src_uid": previous_uid, "src_nid": previous_nid,
                                                                       "relation": "precedes", "tgt_uid": n["uid"],
                                                                       "tgt_nid": n["nid"]}])], ignore_index=True)
            previous_uid = n["uid"]
            previous_nid = int(n["nid"])
    ...
    g.add_node(last_nid, "jury", color=project_color, group="project", shape="image",
               image=OCGraphsIconsURL["project_jury"], physics=False, x=px, y=py)
    g.add_edge(previous_nid, last_nid, length=50)  # ,hidden=True
    ph += 30
    px, py = climb_project_mountain(ph, hmax, ymax, shape)
    last_nid = aia.record_object_id(str(path_id) + "-Bravo")
    g.add_node(last_nid, "Bravo!", color=project_color, group="project", shape="image",
               image=OCGraphsIconsURL["project_success"], physics=False, x=px, y=py)
    ...
    for i, e in dfpc.iterrows():
        ...
        skeleton_edges = pd.concat([skeleton_edges, pd.DataFrame([{"src_uid": e["src_uid"], "src_nid": e["src_nid"],
                                                                   "relation": "project_requires",
                                                                   "tgt_uid": e["tgt_uid"], "tgt_nid": e["tgt_nid"]}])],
                                   ignore_index=True)
        if (i_read_my_courses_once and e["course_id"] in already_visited_courses):
            ...
            g.add_edge(int(e["src_nid"]), int(e["tgt_nid"]), label=str(int(e["course_duration_hours"])) + "h",
                       color={'color': link_colors["primary_requires"], 'inherit': 'false'}, title="1st req")
    ...
    for i, e in dfcc.iterrows():
        ...
        skeleton_edges = pd.concat([skeleton_edges, pd.DataFrame([{"src_uid": e["src_uid"], "src_nid": e["src_nid"],
                                                                   "relation": e["relation"], "tgt_uid": e["tgt_uid"],
                                                                   "tgt_nid": e["tgt_nid"]}])], ignore_index=True)
    ...
    dfccm = dfcc.merge(skeleton_edges[["src_uid", "tgt_uid", "parenthood", "parenthood_uid"]],
                       on=["src_uid", "tgt_uid"], how="left")
    ...
    for depth in range(1, len(courses_layers)):
        ...
        for i, e in dfccm[
            dfccm["src_course_id"].isin(previous_layer) & dfccm["tgt_course_id"].isin(cset)].dropna().iterrows():
            ...
            if (show_titles):
                g.add_node(int(e["src_nid"]), e["src_course_title"], size=e["src_course_difficulty_grade"] * 10,
                           group="course", \
                           color=course_color, shape="image", image=srcimage, \
                           title="<a href=\"" + e["src_course_url"] + "\" target=\"_blank\">" + e[
                               "src_course_title"] + "</a>")
                g.add_node(int(e["tgt_nid"]), e["tgt_course_title"], size=e["tgt_course_difficulty_grade"] * 10,
                           group="course", \
                           color=course_color, shape="image", image=tgtimage, \
                           title="<a href=\"" + e["tgt_course_url"] + "\" target=\"_blank\">" + e[
                               "tgt_course_title"] + "</a>")
            else:
                g.add_node(int(e["src_nid"]), e["src_course_title"], size=e["src_course_difficulty_grade"] * 10,
                           group="course", \
        ...
        if (show_only_requirement_relations == False):
            for i, e in dfccm[
                ...
                if (e["parenthood"] > show_relations_max_distance and create_pseudo_nodes_over_max_distance == True):
                    # create smthng
                    #                     print("adding longlink ", int(e["parenthood"]), e["src_course_title"],int(e["src_nid"]), e["tgt_course_title"], int(e["tgt_nid"]))
                    add_pseudo_nodes_between(g, aia, int(e["src_nid"]), int(e["tgt_nid"]), e["src_uid"], e["tgt_uid"],
                                             show_relations_max_distance, int(e["parenthood"]),
                                             color=link_colors["long_link"], show_titles=show_titles)
                else:
                    edge_title = "2nd ref / " + str(int(e["parenthood"])) + " : " + str(e["parenthood_uid"])
                    g.add_edge(int(e["src_nid"]), int(e["tgt_nid"]),
                               label=str(int(e["tgt_course_duration_hours"])) + "h",
                               color={'color': color, 'inherit': 'false'}, title=edge_title)
                already_added_relations.append((int(e["src_nid"]), int(e["tgt_nid"])))
    ...
    for i, e in dfccm.dropna().iterrows():
        ...
        if (e["parenthood"] > show_relations_max_distance and create_pseudo_nodes_over_max_distance == True):
            ...
            #             print("adding longlink ", int(e["parenthood"]), e["src_course_title"],int(e["src_nid"]), e["tgt_course_title"], int(e["tgt_nid"]))
            add_pseudo_nodes_between(g, aia, int(e["src_nid"]), int(e["tgt_nid"]), e["src_uid"], e["tgt_uid"],
                                     show_relations_max_distance, int(e["parenthood"]), color=link_colors["long_link"],
...
def build_courses_graph(topic_id=0, language='-', path_topic_id=0, path_language='-', \
    ...
    dfpc = ocd.OC_ProjectsCoursesLinks[ocd.OC_ProjectsCoursesLinks["path_id"].isin(dfpa.path_id.unique()) & \
                                       ocd.OC_ProjectsCoursesLinks["course_id"].isin(
                                           filter_courses)]  # courses referenced/required by the project
    ...
    dfcc = ocd.OC_CoursesLinks[(ocd.OC_CoursesLinks["src_course_id"].isin(filter_courses)) & \
                               (ocd.OC_CoursesLinks["tgt_course_id"].isin(filter_courses))].drop_duplicates(
        subset=["src_course_id", "tgt_course_id", "relation"])
    ...
    dfpa = dfpa[
        ["topic_id", "topic_color", "path_id", "path_title", "path_language", "path_level", "path_duration_months",
         "path_illustration", "path_url"]].copy()
    ...
    # courses
    dfc = dfc[['topic_id', "topic_color", 'course_id', 'course_title', 'course_difficulty', 'course_duration_hours',
               'course_url', "status"]].copy()
    # courses-courses edges
    # display(dfcc[dfcc.duplicated(subset=["src_course_id","tgt_course_id","relation"],keep=False)])
    ...
    dfcc = dfcc[
        ['src_course_id', 'relation', 'tgt_course_id', 'course_title', 'course_difficulty', 'course_duration_hours',
         "topic_id", "topic_color"]].copy()
    dfcc.columns = ['src_course_id', 'relation', 'tgt_course_id', \
                    'src_course_title', 'src_course_difficulty', 'src_course_duration_hours', "src_topic_id",
                    "src_topic_color"]
    ...
    dfcc.columns = ['src_course_id', 'relation', 'tgt_course_id', \
                    'src_course_title', 'src_course_difficulty', 'src_course_duration_hours', "src_topic_id",
                    "src_topic_color", \
                    'tgt_course_title', 'tgt_course_difficulty', 'tgt_course_duration_hours', "tgt_topic_id",
                    "tgt_topic_color"]
    ...
    for i, n in dfpa.iterrows():
        ...
        if (show_titles):
            g.add_node(n["nid"], n["path_title"], color=color, shape=path_shape, size=path_size, \
                       physics=True,
                       title="<a href=\"" + n["path_url"] + "\" target=\"_blank\">" + n["path_title"] + "</a>")
        else:
            ...
            for ii, nn in dfpr[dfpr["path_id"].isin([n["path_id"]])].iterrows():
                ...
                if fs not in added_edges:
                    ...
                    if (show_my_way == True) and (nn["status"] != "none"):
                        color = status_colors[nn["status"]]
                    color = get_darker_color(color, 30)
                    g.add_edge(previous_nid, int(nn["nid"]), label=str(int(nn["project_duration_hours"])) + "h",
                               color={'color': color, 'inherit': 'false'}, value=2)
                previous_uid = nn["uid"]
    ...
    for i, n in dfc.iterrows():
        ...
        if (show_titles):
            g.add_node(n["nid"], n["course_title"], color=color, size=n["course_difficulty_grade"] * 10 * size_coeff, \
                       ...
                       title="<a href=\"" + n["course_url"] + "\" target=\"_blank\">" + n["course_title"] + "</a>")
        else:
            g.add_node(n["nid"], n["course_title"], color=color, size=n["course_difficulty_grade"] * 10 * size_coeff, \
                       shape=course_shape, physics=True)
        ...
        for i, e in dfpc.iterrows():
            ...
            try:
                ...
                if fs not in added_edges:
                    ...
                    if (palette == "Topic"):
                        color = e["topic_color"]
                    g.add_edge(int(e["src_nid"]), int(e["tgt_nid"]), label=str(duration) + "h",
                               color={'color': color, 'inherit': 'false'}, value=2)
            except:
        ...
        for i, e in dfpc.iterrows():
            ...
            try:
                ...
                if fs not in added_edges:
                    ...
                    if (palette == "Topic"):
                        color = e["topic_color"]
                    g.add_edge(int(e["path_nid"]), int(e["tgt_nid"]), label=str(duration) + "h",
                               color={'color': color, 'inherit': 'false'})
            except:
    ...
    for i, e in dfcc.iterrows():
        ...
        if fs not in added_edges:
            ...
            if (edge_value == 0):
                ...
                g.add_edge(int(e["src_nid"]), int(e["tgt_nid"]), label=str(duration) + "h",
                           color={'color': color, 'inherit': 'false'}, value=edge_value)
