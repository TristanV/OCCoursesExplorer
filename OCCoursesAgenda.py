...
from OCCoursesConfig import np, OCGraphsIconsURL
...
def get_degree_in(course_id, dfcc, dfc):
    return len(dfcc[dfcc["tgt_course_id"].isin([course_id]) & ~dfcc["src_course_id"].isin([course_id]) & dfcc[
        "src_course_id"].isin(dfc.course_id.values)])


# end function

def get_degree_out(course_id, dfcc, dfc):
    return len(dfcc[dfcc["src_course_id"].isin([course_id]) & ~dfcc["tgt_course_id"].isin([course_id]) & dfcc[
        "tgt_course_id"].isin(dfc.course_id.values)])
...
def get_project_dependencies(path_id, project_number, required_max_depth, references_max_depth, courses_exclusion_set):
    ...
    dfpc = ocd.OC_ProjectsCoursesLinks[ocd.OC_ProjectsCoursesLinks["path_id"].isin([path_id]) & \
                                       ocd.OC_ProjectsCoursesLinks["project_number"].isin(
                                           [project_number])]  # courses referenced/required by the project
    ...
    dfcc = ocd.OC_CoursesLinks[(ocd.OC_CoursesLinks["src_course_id"].isin(path_courses)) & \
                               (ocd.OC_CoursesLinks["tgt_course_id"].isin(path_courses))].drop_duplicates(
    ...
    dfcc = dfcc[
        ['src_course_id', 'relation', 'tgt_course_id', 'course_title', 'course_difficulty', 'course_duration_hours',
         'status']].copy()
    dfcc.columns = ['src_course_id', 'relation', 'tgt_course_id', 'src_course_title', 'src_course_difficulty',
                    'src_course_duration_hours', 'src_status']
    ...
    dfcc = dfcc.sort_values(by=["tgt_course_duration_hours", "tgt_course_difficulty_grade", "src_course_duration_hours",
                                "src_course_difficulty_grade"]).drop_duplicates(
        subset=["src_course_id", "tgt_course_id", "relation"])
    ...
    rank_req_dfcc = dfcc[dfcc["relation"].isin(["requires"])][
        ["src_course_id", "tgt_course_id"]].drop_duplicates().copy()
    rank_ref_dfcc = dfcc[~dfcc["relation"].isin(["requires"])][
        ["src_course_id", "tgt_course_id"]].drop_duplicates().copy()
    ...
    while len(rank_req_dfc) > 0:
        ...
        if len(rank_req_dfc) > 0:
            rank_req_dfc["degree_req_out"] = rank_req_dfc.apply(
                lambda x: get_degree_out(x['course_id'], rank_req_dfcc, rank_req_dfc), axis=1)
        rank += 1
    # end while  
    ...
    while len(rank_ref_dfc) > 0:
        ...
        if len(rank_ref_dfc) > 0:
            rank_ref_dfc["degree_ref_out"] = rank_ref_dfc.apply(
                lambda x: get_degree_out(x['course_id'], rank_ref_dfcc, rank_ref_dfc), axis=1)
        rank += 1
    # end while  
    ...
    return dfc.sort_values(
        by=["rank_req", "rank_ref", "course_difficulty_grade", "course_duration_hours", "degree_req_out",
            "degree_ref_out"]), dfcc, path_critical_courses
...
# --------------------------------------------------------------------------------------------------
# --------------------------------------------------------------------------------------------------
def build_path_agenda_html(path_id, required_max_depth=1, references_max_depth=0, show_my_way=True, show_details=False,
                           options=None):
    ...
    dfpr = ocd.OC_Projects[ocd.OC_Projects["path_id"].isin(dfpa.path_id.unique())].sort_values(
        by="project_number")  # projects nodes
    ...
    dfpr = dfpr[["path_id", "project_number", "project_title", "project_description", "project_duration_hours",
                 "status"]].sort_values(by="project_number")
    ...
    if len(dfpr) == 0:
        ...
        ht += "<p>Language : " + path["path_language"] + " / Duration : " + str(
            path["path_duration_months"]) + "m / Level : " + str(path["path_level"]) + "</p>"
        if show_details:
    ...
    for ii, nn in dfpr[dfpr["path_id"].isin([path_id])].iterrows():
        ...
        if show_details:
            ...
            skills = ocd.OC_ProjectsSkills[ocd.OC_ProjectsSkills["path_id"].isin([nn["path_id"]]) & \
                                           ocd.OC_ProjectsSkills["project_number"].isin(
                                               [nn["project_number"]])].skill.values
            if (len(skills) > 0):
                ht += "<p>Skills:</p>"
                ht += "<ul>"
                for s in skills:
        ...
        dfc, dfcc, path_critical_courses = get_project_dependencies(path_id, nn["project_number"], required_max_depth,
                                                                    references_max_depth, set())
        ...
        if (type(dfc) != type(None)) and len(dfc) > 0:
            ...
            for i, r in dfc.iterrows():
                ...
                if r[
                    "course_id"] in courses_exclusion_set:  # if the course has already been seen before, we do not count it twice in the stats, but we still show it in the schedule
                ...
                if (ignore_this_course):
                    ...
                    # ---------------
                    ht += "<td colspan=2>"
                    ht += str(r["course_id"]) + "-<a href=\"" + r["course_url"] + "\" target=\"_blank\">" + r[
                        "course_title"] + "</a>"
                    ...
                    if show_details:
                        ...
                        for ip, rp in dfparts.iterrows():
                            ...
                            dfchapters = ocd.OC_CoursesChapters[
                                ocd.OC_CoursesChapters["course_id"].isin([r["course_id"]]) & ocd.OC_CoursesChapters[
                                    "part_number"].isin([rp["part_number"]])]
                            ht += "<ol>"
                            for ic, rc in dfchapters.iterrows():
                                ht += "<li>" + rc["chapter_title"] + "</li>"
                            ht += "</ol>"
                        ht += "</ol>"
                        skills = ocd.OC_CoursesSkills[
                            ocd.OC_CoursesSkills["course_id"].isin([r["course_id"]])].skill.values
                        if (len(skills) > 0):
                            ht += "<p>Skills:</p>"
                            ht += "<ul>"
                            for s in skills:
                ...
                required_courses = dfcc[
                    dfcc["src_course_id"].isin([r["course_id"]]) & dfcc["relation"].isin(["requires"]) & ~dfcc[
                        "tgt_course_id"].isin([r["course_id"]])].merge(dfc, left_on="tgt_course_id",
                                                                       right_on="course_id", how="left").dropna(
                    subset=["course_id"])
                if (len(required_courses) > 0):
                    ...
                    for ii, rr in required_courses.iterrows():
                        ...
                        ht += "<br/><span class=\"" + course_status_class + "\">" + str(
                            rr["course_id"]) + "</span>-<a href=\"" + rr["course_url"] + "\" target=\"_blank\">" + rr[
                                  "course_title"] + "</a>"
                    ht += "</div>"

                references_courses = dfcc[
                    dfcc["src_course_id"].isin([r["course_id"]]) & ~dfcc["relation"].isin(["requires"]) & ~dfcc[
                        "tgt_course_id"].isin([r["course_id"]]) & ~dfcc["tgt_course_id"].isin(
                        required_courses.tgt_course_id.values)].merge(dfc, left_on="tgt_course_id",
                                                                      right_on="course_id", how="left").dropna(
                    subset=["course_id"])
                ...
                required_courses = dfcc[
                    dfcc["tgt_course_id"].isin([r["course_id"]]) & dfcc["relation"].isin(["requires"]) & ~dfcc[
                        "src_course_id"].isin([r["course_id"]])].merge(dfc, left_on="src_course_id",
                                                                       right_on="course_id", how="left").dropna(
                    subset=["course_id"])
                ...
                references_courses = dfcc[
                    dfcc["tgt_course_id"].isin([r["course_id"]]) & ~dfcc["relation"].isin(["requires"]) & ~dfcc[
                        "src_course_id"].isin([r["course_id"]]) & ~dfcc["src_course_id"].isin(
                        required_courses.src_course_id.values)].merge(dfc, left_on="src_course_id",
                                                                      right_on="course_id", how="left").dropna(
                    subset=["course_id"])
                if (len(references_courses) > 0):
        ...
        # --------------- Totals
        ht += "<thead><tr>"
        ht += "<th colspan=3>Project #" + str(nn[
                                                  "project_number"]) + "</th><th>courses</th><th>courses duration</th><th>project duration</th><th>percent</th>"
        ht += "</tr>"
        if (show_my_way):
            if (project_courses_unbooked > 0):
                ht += "<tr>"
                ht += "<td colspan='3' class='noborder'></td><td  class='border'>" + str(
                    project_courses_unbooked) + " unbooked</td><td class='border'>" + str(
                    project_scheduled_unbooked_duration) + " h</td><td class='noborder'></td><td class='border'>" + str(
                    int(100 * project_scheduled_unbooked_duration / project_allocated_duration)) + "%</td>"
                ht += "</tr>"
            if (project_courses_booked > 0):
                ht += "<tr>"
                ht += "<td colspan='3' class='noborder'></td><td class='border'>" + str(
                    project_courses_booked) + " booked</td><td class='border'>" + str(
                    project_scheduled_booked_duration) + " h</td><td class='noborder'></td><td class='border'>" + str(
                    int(100 * project_scheduled_booked_duration / project_allocated_duration)) + "%</td>"
                ht += "</tr>"
            if (project_courses_in_progress > 0):
                ht += "<tr>"
                ht += "<td colspan='3' class='noborder'></td><td class='border'>" + str(
                    project_courses_in_progress) + " in progress</td><td class='border'>" + str(
                    project_scheduled_in_progress_duration) + " h</td><td class='noborder'></td><td class='border'>" + str(
                    int(100 * project_scheduled_in_progress_duration / project_allocated_duration)) + "%</td>"
                ht += "</tr>"
            if (project_courses_done > 0):
                ht += "<tr>"
                ht += "<td colspan='3' class='noborder'></td><td class='border'>" + str(
                    project_courses_done) + " achieved</td><td class='border'>" + str(
                    project_scheduled_done_duration) + " h</td><td class='noborder'></td><td class='border'>" + str(
                    int(100 * project_scheduled_done_duration / project_allocated_duration)) + "%</td>"
                ht += "</tr>"
        ht += "<tr>"
        ht += "<td colspan=3 class='noborder'></td>"
        ht += "<th>" + str(project_courses_count) + " courses<br/>incl " + str(
            project_courses_critical_count) + " critical courses</th>"
        ht += "<th>" + str(project_scheduled_duration) + " h<br/>" + str(
            project_scheduled_critical_duration) + " h</th>"
        ht += "<th>" + str(project_allocated_duration) + " h</th>"
        ht += "<th>" + str(int(100 * project_scheduled_duration / project_allocated_duration)) + "%<br/>" + str(
            int(100 * project_scheduled_critical_duration / project_allocated_duration)) + " %</th>"
        ht += "</tr></thead>"
        ht += "</table>"
    ...
    if (show_my_way):
        if (total_courses_unbooked > 0):
            ht += "<tr>"
            ht += "<td class='border'>" + str(total_courses_unbooked) + " unbooked</td><td class='border'>" + str(
                total_scheduled_unbooked_duration) + " h</td><td class='noborder'></td><td class='border'>" + str(
                int(100 * total_scheduled_unbooked_duration / total_allocated_duration)) + "%</td>"
            ht += "</tr>"
        if (total_courses_booked > 0):
            ht += "<tr>"
            ht += "<td class='border'>" + str(total_courses_booked) + " booked</td><td class='border'>" + str(
                total_scheduled_booked_duration) + " h</td><td class='noborder'></td><td class='border'>" + str(
                int(100 * total_scheduled_booked_duration / total_allocated_duration)) + "%</td>"
            ht += "</tr>"
        if (total_courses_in_progress > 0):
            ht += "<tr>"
            ht += "<td class='border'>" + str(total_courses_in_progress) + " in progress</td><td class='border'>" + str(
                total_scheduled_in_progress_duration) + " h</td><td class='noborder'></td><td class='border'>" + str(
                int(100 * total_scheduled_in_progress_duration / total_allocated_duration)) + "%</td>"
            ht += "</tr>"
        if (total_courses_done > 0):
            ht += "<tr>"
            ht += "<td class='border'>" + str(total_courses_done) + " achieved</td><td class='border'>" + str(
                total_scheduled_done_duration) + " h</td><td class='noborder'></td><td class='border'>" + str(
                int(100 * total_scheduled_done_duration / total_allocated_duration)) + "%</td>"
            ht += "</tr>"
    ht += "<tr>"
    ht += "<th>" + str(total_courses_count) + " courses<br/>incl " + str(
        total_courses_critical_count) + " critical courses</th>"
    ...
    ht += "<th>" + str(int(100 * total_scheduled_duration / total_allocated_duration)) + "%<br/>" + str(
        int(100 * total_scheduled_critical_duration / total_allocated_duration)) + " %</th>"
    ht += "</tr></thead>"
    ht += "</table>"
    ht += "<br/>"
    ht += "<br/>"
