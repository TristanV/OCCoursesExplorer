...
OC_Paths_cols               = ["topic_id","path_id","path_name","path_date","path_title","path_description","path_language","path_level","path_duration_months",\
                               "path_employment_warranty","path_partner","path_illustration","path_url","path_certification_url"]
...
OC_Projects_cols            = ["path_id","project_id","project_number","project_name","project_title","project_description","project_duration_hours","project_illustration","project_url"]
...
OC_Courses_cols             = ["topic_id","course_id","course_name","course_date","course_title","course_description","course_language","course_difficulty",\
                               "course_duration_hours","course_author","course_partner","course_illustration","course_is_certified","course_url","course_activities_count","course_exercises_count"]
...
OC_MyCourses_cols             = ["titreCours","inscription","progression","moyenneDeClasse","duree","difficulte","nbChapitres","ratioQuizEvaluation","nbEvaluations","derniereMiseAJour","idCours"]
...
def load_dataset(filename,expected_columns,delimiter="\t"):
    ...
    if not fileObj.is_file(): 
        console_log ("The file "+occ.DatasetsFolder+filename+" could not be loaded! It will be necessary to rebuild it.")
    ...
    if columns_OK==False:
        console_log ("Because of missing columns, the file "+occ.DatasetsFolder+filename+" is partially broken! It will be necessary to rebuild it.")
        if (len(df)==0):
            return pd.DataFrame(columns=expected_columns)
        else:
            return df
...
def save_OC_datasets():
    ...
    OC_MyCourses.to_csv(occ.DatasetsFolder+"my_courses.csv",index=False,sep=",") #note the ',' to remain fully compatible with the OC exercise
...
#     display(ocd.OC_CoursesLinks[ocd.OC_CoursesLinks["tgt_course_id"].isin([0])].head())
    ...
    dfd = OC_CoursesLinks[OC_CoursesLinks.duplicated(['src_course_id','src_chapter_id',"relation","href"], keep=False)]
    duplicates = len(dfd)
