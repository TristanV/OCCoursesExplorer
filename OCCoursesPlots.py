...
from OCCoursesConfig import pd, plt, sns, math
...
def draw_micro_courses_details_plot():
    ...
    df = df[["src_topic_id", "src_topic_uid", "src_course_id", "src_chapter_id", 'tgt_topic_id', "tgt_course_id",
             "tgt_chapter_id"]]
    df = df.merge(dt, left_on='tgt_topic_id', right_on='topic_id', how='left')
    df["tgt_topic_uid"] = df["topic_uid"]

    df = df[["src_topic_id", "src_topic_uid", "src_course_id", "src_chapter_id", 'tgt_topic_id', 'tgt_topic_uid',
             "tgt_course_id", "tgt_chapter_id"]].dropna()
    ...
    q = sns.scatterplot(ax=ax, x="src_topic_uid", y="tgt_topic_uid", hue="tgt_topic_uid", size="ln", sizes=(10, 300),
                        data=ddf, legend=False, palette=sns.color_palette(dt.topic_color.values))
    sns.despine()
    sns.set_style("white")
    plt.axis('off')
    q.set(xticklabels=[])
...
def draw_micro_paths_plot():
    ...
    p = sns.catplot(x="topic_uid", kind="count", data=df, legend=False, palette=sns.color_palette(dt.topic_color.values), height=5, aspect=5.3)  # height=3,aspect=6.3
    sns.despine()
    plt.axis('off')
    p.set(xticklabels=[])
    p.set(yticklabels=[])
...
def draw_micro_projects_plot():
    ...
    p = sns.catplot(x="topic_uid", kind="count", data=df, legend=False, palette=sns.color_palette(dt.topic_color.values), height=5.6, aspect=3.8)  # ,height=5,aspect=3.8
    sns.despine()
    plt.axis('off')
    p.set(xticklabels=[])
    p.set(yticklabels=[])
