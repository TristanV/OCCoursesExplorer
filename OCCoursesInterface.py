...
from OCCoursesConfig import widgets,display, clear_output, HTML, IFrame ,Image, OCGraphsIconsLocal,time
...
status_output = widgets.Output(layout={'border': '1px solid #CCC','width':'99%','height':'70px','overflow':'auto'})
status_progress = widgets.IntProgress(description="loading",value=25,min=0,max=100,layout={'border': '1px solid #CCC','width':'99%','height':'30px'})
...
paths_items =widgets.HBox([paths_topic_selector, paths_language_selector, paths_level_selector, paths_duration_selector,paths_link_area])
...
#         menu_tab.layout=widgets.Layout(width='100%', height=str(projects_height_slider.value+180)+'px')
    ...
    if (w["owner"] == projects_topic_selector):
        df=ocd.OC_Paths[["path_title","path_language","path_id"]][ocd.OC_Paths["topic_id"].isin([topic_id])].sort_values(by=["path_language","path_title"])
        opt=[("-",0)]
        for i,r in df.iterrows():
...
projects_out.layout=widgets.Layout(width='98%', height=str(projects_height_slider.value+70)+'px')
projects_box=widgets.VBox([projects_items_r1,projects_items_r2,widgets.HBox([projects_height_slider,projects_out])]) 
...
information_items =widgets.HBox([information_minidoc_togglebutton,information_readme_togglebutton,information_licence_togglebutton])
...
def wcell(content,width='200px',height='90%',color='#111',border='1px solid #CCC'):
    cell = widgets.HTML("<center><font color='"+color+"'>"+content+"</font></center>",layout={'border': border,'width':width,'height':height})
    return cell
# fin fonction 
...
def wheadcell(content,width='200px',border='2px solid #CCC',icon=""):
    ...
    if icon!="":
        ...
        cell = widgets.HBox([widgets.HTML("<div align='center'><b>"+t+content+"</b></div>",box_style='success',layout={'border': border,'width':width,'height':'90%'}),wimg])
    else:
        cell = widgets.HTML("<div align='center'><b>"+t+content+"</b></div>",box_style='success',layout={'border': border,'width':width,'height':'90%'})
    return cell
# fin fonction 
...
def wleftbordercell(content,width='200px',borderleft='1px solid #CCC'):
    cell = widgets.HTML("<div align='center' style='border-left:"+borderleft+";'><center>"+content+"</center></div>",layout={'width':width,'height':'90%'})
    return cell
# fin fonction 
...
def toggle_scrap_buttons(disable = True):
    ...
    buttons =[scrap_OC_Topics_button, scrap_OC_Courses_button, scrap_OC_CoursesDetails_button, scrap_OC_CoursesLinks_button,\
            scrap_OC_Paths_button, scrap_OC_PathsSkills_button, scrap_OC_Projects_button, scrap_OC_ProjectsCoursesLinks_button ,\
            scrap_OC_MyCourses_button, scrap_OC_RepairCoursesLinks_button]
...
def display_data_section():
    ...
    with data_OC_out:
        ...
        #--- head
        cells=[]
        cells.append(wheadcell("<font color='#7451eb' size='4'>OpenClassrooms Courses</font>",width='250px',border='none'))
        ...
        cells.append(wcell(get_data_status(ocd.OC_CoursesChapters,ocd.OC_CoursesChapters_cols),width='80px',color=color))    
        cells.append(wleftbordercell("<i>(See above)</i>",width='220px',borderleft='20px solid #EEE'))
        cells.append(wemptycell(width='80px'))
        rows.append(wrow(cells))            
        ...
        cells.append(wcell(get_data_status(ocd.OC_CoursesLinks,ocd.OC_CoursesLinks_cols,test_courses_links=True),width='80px',color=color))    
        ...
        if (missing_cl+duplicates_cl>0):
            ...
            cells.append(wemptycell(width='80px'))
            scrap_OC_RepairCoursesLinks_button=widgets.Button(description="Repair "+str(missing_cl+duplicates_cl)+" links!",
                                                              icon="medkit",title=str(missing_cl)+" links are missing and "+str(duplicates_cl)+" are duplicated" )
        ...
        cells.append(wcell(get_data_status(ocd.OC_ProjectsSkills,ocd.OC_ProjectsSkills_cols),width='80px',color=color))    
        cells.append(wleftbordercell("<i>(See above)</i>",width='220px',borderleft='20px solid #EEE'))
        cells.append(wemptycell(width='80px'))
        rows.append(wrow(cells))        
        ...
        cells.append(wcell(get_data_status(ocd.OC_ProjectsCoursesLinks,ocd.OC_ProjectsCoursesLinks_cols),width='80px',color=color))    
...
#         display(wtable(rows))
        ...
        rows=[]
        cells=[]
        cells.append(wheadcell("<font color='#7451eb' size='3'>--> Write OpenClassrooms Courses to CSV --> </font>",width='440px',border='none'))
